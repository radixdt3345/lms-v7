...
const sampleResult = {
  items: [{
    id: 'aaa-111', actorUserId: 'uid-1', actorName: 'admin@example.com',
    actionType: 'CREATE', recordType: 'Employee', recordId: 'emp-1',
    oldValue: null, newValue: '{"name":"Alice"}', ipAddress: null,
...
function makeStore() {
  return configureStore({
    reducer: { authentication: authenticationReducer, auditLog: auditLogReducer },
  });
}
function renderPage() {
  return render(<Provider store={makeStore()}><AuditTrailPage /></Provider>);
}
...
describe('AuditTrailPage', () => {
  ...
  it('renders the audit trail table', async () => {
    renderPage();
    await waitFor(() => expect(screen.getByTestId('audit-trail-table')).toBeInTheDocument());
  });
  ...
  it('displays rows returned by the API', async () => {
    ...
    renderPage();
    await waitFor(() => expect(screen.getByTestId('row-aaa-111')).toBeInTheDocument());
  });
});
